		...
		try {
			...
			for(Map.Entry<String, String> entry : modelMap.entrySet()) {
				...
				if(key.contains("rubrica_descripcion")) {
					...
					if(value.equals("")) {
						throw new Exception("Por favor completar la descripcion de las r�bricas");
					}
					rubricas.put(Integer.parseInt(infoItem[2]), value);
				}else if(key.contains("id_tipo_rubrica")) {
					
					if(value.equals("") || value.equals("-1")) {
						throw new Exception("Por favor completar los tipos de las r�bricas");
					}
			...
			object.put("message", "Se ha creado la r�brica correctamente");
		} catch (Exception e) {
			e.printStackTrace();
